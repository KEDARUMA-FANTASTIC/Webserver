...
unsigned char equalsConstantTime(const String &s1, const String &s2) {
  ...
  unsigned char equalcond = (equalchars == s1.length());
...
DEBUG_OUTPUT.println("### WebServer::_handleRequest 1");
  if (!handled && _notFoundHandler) {
DEBUG_OUTPUT.println("### WebServer::_handleRequest 2");
    _notFoundHandler();
    handled = true;
  }
  if (!handled) {
